...
const nameMagica = 'Mágica Instantânea';
let liked = false;
const SEVEN_SIX_EIGTH = 768;
const cardSet = [
  {
    imageUrl: 'img/card2.jpg',
    ...
    type: nameMagica },
  {
    imageUrl: 'img/card4.jpg',
    englishName: 'Coordinated Assault',
    name: 'Assalto Coordenado',
    mana: 1,
    color: 'Vermelho',
    manaImage: 'img/redMana.png',
    type: 'Instantânea' },
  {
...
const adjustMenuOnResize = () => {
  if (window.innerWidth >= SEVEN_SIX_EIGTH) {
    navContainer.style.display = 'flex';
  } else {
...
const addManaImg = (number, url, color) => {
  ...
  for (i = 0; i < number; i++) {
    manaDetails
      .innerHTML += `<li><img src="${url}" alt="${color}" class="miniMana"/></li>`;
  }
};
...
const createListImg = (cardSet2) => {
  imgList.innerHTML = '';

  cardSet2.forEach((card) => {
    imgList.innerHTML += `<li><img src="${card.imageUrl}" 
    alt="${card.name}" class="miniCard"/></li>`;
  });
};
